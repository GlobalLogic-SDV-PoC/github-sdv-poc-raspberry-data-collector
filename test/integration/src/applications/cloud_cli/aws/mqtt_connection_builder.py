    ...
    **kwargs,
    ...
    if port == 443 and awscrt.io.is_alpn_available() and use_custom_authorizer is False:
        tls_ctx_options.alpn_list = (
            ["http/1.1"] if use_websockets else ["x-amzn-mqtt-ca"]
        )
    ...
    socket_options.keep_alive = _get(
        kwargs, "tcp_keep_alive", _get(kwargs, "tcp_keepalive", False)
    )

    socket_options.keep_alive_timeout_secs = _get(
        kwargs,
        "tcp_keep_alive_timeout_secs",
        _get(kwargs, "tcp_keepalive_timeout_secs", 0),
    )

    socket_options.keep_alive_interval_secs = _get(
        kwargs,
        "tcp_keep_alive_interval_secs",
        _get(kwargs, "tcp_keepalive_interval_secs", 0),
    )

    socket_options.keep_alive_max_probes = _get(
        kwargs, "tcp_keep_alive_max_probes", _get(kwargs, "tcp_keepalive_max_probes", 0)
    )
    ...
    proxy_options = kwargs.get(
        "http_proxy_options", kwargs.get("websocket_proxy_options", None)
    )
...
def mtls_from_path(cert_filepath, pri_key_filepath, **kwargs) -> awscrt.mqtt.Connection:
    ...
    tls_ctx_options = awscrt.io.TlsContextOptions.create_client_with_mtls_from_path(
        cert_filepath, pri_key_filepath
    )
...
def mtls_from_bytes(cert_bytes, pri_key_bytes, **kwargs) -> awscrt.mqtt.Connection:
    ...
    """
    _check_required_kwargs(**kwargs)
    tls_ctx_options = awscrt.io.TlsContextOptions.create_client_with_mtls(
        cert_bytes, pri_key_bytes
    )
    return _builder(tls_ctx_options, **kwargs)
...
def mtls_with_pkcs12(
    *, pkcs12_filepath: str, pkcs12_password: str, **kwargs
) -> awscrt.mqtt.Connection:
    """
    This builder creates an :class:`awscrt.mqtt.Connection`, configured for an mTLS MQTT connection to AWS IoT,
    ...
    tls_ctx_options = awscrt.io.TlsContextOptions.create_client_with_mtls_pkcs12(
        pkcs12_filepath=pkcs12_filepath, pkcs12_password=pkcs12_password
    )
    return _builder(tls_ctx_options, **kwargs)


def mtls_with_windows_cert_store_path(
    *, cert_store_path: str, **kwargs
) -> awscrt.mqtt.Connection:
    """
    This builder creates an :class:`awscrt.mqtt.Connection`, configured for an mTLS MQTT connection to AWS IoT,
    ...
    tls_ctx_options = (
        awscrt.io.TlsContextOptions.create_client_with_mtls_windows_cert_store_path(
            cert_store_path
        )
    )
...
def websockets_with_default_aws_signing(
    region, credentials_provider, websocket_proxy_options=None, **kwargs
) -> awscrt.mqtt.Connection:
    """
    ...
    def _sign_websocket_handshake_request(transform_args, **kwargs):
        ...
        try:
            ...
            signing_future = awscrt.auth.aws_sign_request(
    ...
    return websockets_with_custom_handshake(
        _sign_websocket_handshake_request, websocket_proxy_options, **kwargs
    )


def websockets_with_custom_handshake(
    websocket_handshake_transform, websocket_proxy_options=None, **kwargs
) -> awscrt.mqtt.Connection:
    """
...
def _with_custom_authorizer(
    ...
    **kwargs,
    ...
    if auth_token_key_name is not None and auth_token_value is not None:
        username_string = _add_to_username_parameter(
            username_string, auth_token_value, auth_token_key_name + "="
        )
...
def new_default_builder(**kwargs) -> awscrt.mqtt.Connection:
    ...
    return _builder(tls_ctx_options=tls_ctx_options, use_websockets=False, **kwargs)
